...
class Bottleneck_encoder(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(Bottleneck_encoder, self).__init__()
...
class Bottleneck_decoder(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, output_padding=0, upsample=None):
        super(Bottleneck_decoder, self).__init__()
        self.conv1 = nn.ConvTranspose2d(inplanes, planes, kernel_size=1, output_padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.ConvTranspose2d(planes, planes, kernel_size=3, output_padding=0, stride=stride,
                                        padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.ConvTranspose2d(planes, planes * 4, kernel_size=1, output_padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * 4)
        self.relu = nn.ReLU(inplace=True)
        self.upsample = upsample
        self.stride = stride
...
class joint_model_1(nn.Module):
    ...
    def __init__(self, block_encoder, block_decoder, layers):
        ...
        self.fc = nn.Linear(512 * block_encoder.expansion, 10)

        self.inplanes = 64 * block_encoder.expansion
        ...
        self.conv2 = nn.ConvTranspose2d(64, 1, kernel_size=3, stride=2, output_padding=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        de_block_4 = self.decoder_layer4(de_block_3)

        de_block_1_out = self.conv3(de_block_1)
        de_block_2_out = self.conv4(de_block_2)
        de_block_3_out = self.conv5(de_block_3)
        de_block_4_out = self.conv6(de_block_4)

        out_2 = self.conv2(de_block_4)

        return out_1, de_block_1_out, de_block_2_out, de_block_3_out, de_block_4_out, out_2
        # return out_1, out_2
...
class joint_model_2(nn.Module):
    ...
    def __init__(self, block_encoder, block_decoder, layers):
        ...
        self.inplanes = 128 * block_encoder.expansion
...
class joint_model_3(nn.Module):
    ...
    def __init__(self, block_encoder, block_decoder, layers):
        ...
        self.inplanes = 256 * block_encoder.expansion
