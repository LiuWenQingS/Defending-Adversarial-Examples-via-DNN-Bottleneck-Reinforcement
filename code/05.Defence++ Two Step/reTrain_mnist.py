...
def reTrain(best_cla_model_path, best_ae_epoch, round, block, ae_training_set, device_used):
    ...
    parser.add_argument("--checkpoint_path_ae", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/" + ae_training_set +
                                "/Autoencoder/ResNet18/mnist/block_" + str(block) + "/round_" + str(round) +
    ...
    parser.add_argument("--model_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/" + ae_training_set +
                                "/RetrainClassification/ResNet18/mnist/block_" + str(block) +
    ...
    # model_weight_key = model_weight_key[1:-2]
    # model_weight_value = model_weight_value[1:-2]
    # # print(model_weight_key)
    new_dict = {}
    for i in range(len(pretrained_cla_weight_key)):
        if i == 0:
            continue
        elif 0 < i <= (6 + 4 * 6 * block + 6 * (block - 1)):
            new_dict[model_weight_key[i]] = pretrained_ae_weight_value[i - 1]
        else:
    ...
    best_acc_test_clean = 85
    best_acc_test_adv = 0
    best_acc_train_clean = 85
