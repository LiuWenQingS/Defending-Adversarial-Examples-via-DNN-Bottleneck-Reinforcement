...
def train_ae(best_cla_model_path, best_ae_model_path, round, block, ae_training_set, device_used):
    ...
    pretrained_cla_weight_key = pretrained_cla_weight_key[0:-2]
    pretrained_cla_weight_value = pretrained_cla_weight_value[0:-2]
                ...
                with torch.no_grad():
                    ...
                    for batchSize, images_clean, images_adv in load_images(args.input_dir_testSet, batchShape_clean,
                        ...
                        if batch_id_test == 4:
                            ...
                            for i in range(5):
                                a[2][i].clear()
                                a[2][i].imshow(outputs.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                a[2][i].set_xticks(())
                                a[2][i].set_yticks(())
