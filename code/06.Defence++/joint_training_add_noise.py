...
from models.joint_training_cifar10 import joint_model_1, joint_model_2, joint_model_3, BasicBlock_encoder, BasicBlock_decoder
...
def load_images(input_dir, batch_shape):
    ...
    image_noise = np.zeros(batch_shape)
    ...
    for j in range(len(images_clean_list)):
        ...
        image_noise[idx, :, :, :] = ski.random_noise(images_clean_list[j], mode='gaussian', seed=None, clip=True)
        labels[idx] = labels_list[j]

        idx += 1
        if idx == batch_size:
            yield idx, image_clean, image_adv, image_noise, labels
            image_clean = np.zeros(batch_shape)
            image_adv = np.zeros(batch_shape)
            image_noise = np.zeros(batch_shape)
            labels = np.zeros(batch_shape[0])
...
def joint_train(best_cla_model_path, best_ae_model_path, device_used):
    ...
    parser.add_argument("--epochs", type=int, default=100, help="Epoch default:50.")
    ...
    parser.add_argument("--model_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/model/",
                        help="Save model path")
    parser.add_argument("--image_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/image/",
                        help="Save log file")
    parser.add_argument("--acc_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/acc.txt",
                        help="Save accuracy file")
    parser.add_argument("--best_acc_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/best_acc.txt",
                        help="Save best accuracy file")
    parser.add_argument("--log_cla_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/log_cla.txt",
                        help="Save log file")
    parser.add_argument("--log_ae_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/log_ae.txt",
                        help="Save log file")
    parser.add_argument("--loss_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/loss.txt",
                        help="Save log file")
    parser.add_argument("--min_loss_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_6/min_loss.txt",
                        help="Save log file")
            ...
            with open(args.log_ae_file_path, "w")as f3:
                with open(args.loss_file_path, "w")as f4:
                    for epoch in range(1, args.epochs + 1):
                        ...
                        total_train = 0
                        batch = 1
                        for batchSize, images_clean, images_adv, images_noise, labels in load_images(args.input_dir_trainSet,
                                                                                                     batch_shape):
                            start = time.time()
                            # data prepare
                            images_clean = torch.from_numpy(images_clean).type(torch.FloatTensor).to(device)
                            images_noise = torch.from_numpy(images_noise).type(torch.FloatTensor).to(device)
                            ...
                            # forward + backward
                            outputs_cla, outputs_ae = model(images_noise)
                        ...
                        if epoch % 10 == 0:
                            ...
                            with torch.no_grad():
                                ...
                                sum_loss_adv = 0
                                batch_id_test = 1
                                for batchSize, images_clean, images_adv, images_noise, labels in load_images(args.input_dir_testSet,
                                                                                                             batch_shape):
                                    model.eval()
                                    images_clean = torch.from_numpy(images_clean).type(torch.FloatTensor).to(device)
                                    images_noise = torch.from_numpy(images_noise).type(torch.FloatTensor).to(device)
                                    ...
                                    # 测试noise数据集的测试集
                                    outputs_noise_cla, outputs_noise_ae = model(images_noise)
                                    _, predicted_noise_cla = torch.max(outputs_noise_cla.data, 1)
                                    correct_noise_cla += (predicted_noise_cla == labels).sum().item()

                                    loss_noise = criterion_ae(outputs_noise_ae, images_clean)
                                    sum_loss_noise += loss_noise.cpu().data.numpy()
                                    ...
                                    outputs_noise_ae = torch.clamp(outputs_noise_ae, 0, 1)
                                    ...
                                    if batch_id_test == 4:
                                        ...
                                        # second row:noise images
                                        for i in range(5):
                                            a[1][i].clear()
                                            a[1][i].imshow(images_noise.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[1][i].set_xticks(())
                                            a[1][i].set_yticks(())
                                            a[1][i].set_title("Noise Images" + "[" + str(i + 1) + "]")

                                        # third row:adv images
                                        for i in range(5):
                                            a[2][i].clear()
                                            a[2][i].imshow(images_adv.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[2][i].set_xticks(())
                                            a[2][i].set_yticks(())
                                            a[2][i].set_title("Adv Images" + "[" + str(i + 1) + "]")

                                        # fourth row:decoded clean images
                                        for i in range(5):
                                            a[3][i].clear()
                                            a[3][i].imshow(outputs_clean_ae.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[3][i].set_xticks(())
                                            a[3][i].set_yticks(())
                                            a[3][i].set_title("Noise Images Decoded" + "[" + str(i + 1) + "]")

                                        # fifth row:decoded clean images
                                        for i in range(5):
                                            a[4][i].clear()
                                            a[4][i].imshow(outputs_noise_ae.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[4][i].set_xticks(())
                                            a[4][i].set_yticks(())
                                        ...
                                        for i in range(5):
                                            a[5][i].clear()
                                            a[5][i].imshow(outputs_adv_ae.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[5][i].set_xticks(())
                                            a[5][i].set_yticks(())
                                ...
                                print("Clean Test Set Classification Accuracy：%.2f%%" % (correct_clean_cla / total_test * 100))
                                print("Noise Test Set Classification Accuracy：%.2f%%" % (correct_noise_cla / total_test * 100))
                                print("Adv Test Set Classification Accuracy：%.2f%%" % (correct_adv_cla / total_test * 100))
                                acc_clean = correct_clean_cla / total_test * 100
                                acc_noise = correct_noise_cla / total_test * 100
                                acc_adv = correct_adv_cla / total_test * 100
                                # 保存测试集准确率至acc.txt文件中
                                f2.write("Epoch = %03d, Clean Set Accuracy = %.2f%%, Noise Set Accuracy = %.2f%%, Adv Set Accuracy = %.2f%%" %
                                         (epoch, acc_clean, acc_noise, acc_adv))
                                f2.write("\n")
                                f2.flush()

                                print("Clean Test Set AutoEncoder Loss：%.4f" % sum_loss_clean)
                                print("Noise Test Set AutoEncoder Loss：%.4f" % sum_loss_noise)
                                print("Adv Test Set AutoEncoder Loss：%.4f" % sum_loss_ae)
                                # 保存测试集loss至loss.txt文件中
                                f4.write("Epoch = %03d, Clean Set Loss = %.4f, Noise Set Loss = %.4f, Adv Set Loss = %.4f" %
                                         (epoch, sum_loss_clean, sum_loss_noise, sum_loss_ae))
                                f4.write("\n")
                                f4.flush()
