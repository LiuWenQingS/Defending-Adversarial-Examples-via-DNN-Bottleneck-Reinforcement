...
from models.joint_training_lp_hc_cifar import joint_model_1, joint_model_2, joint_model_3, \
...
def joint_train(best_cla_model_path, best_ae_model_path, device_used):
    ...
    parser.add_argument("--model_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/model/",
                        help="Save model path")
    parser.add_argument("--image_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/image/",
                        help="Save log file")
    parser.add_argument("--acc_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/acc.txt",
                        help="Save accuracy file")
    parser.add_argument("--best_acc_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/best_acc.txt",
                        help="Save best accuracy file")
    parser.add_argument("--log_cla_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/log_cla.txt",
                        help="Save log file")
    parser.add_argument("--log_ae_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/log_ae.txt",
                        help="Save log file")
    parser.add_argument("--loss_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/loss.txt",
                        help="Save log file")
    parser.add_argument("--min_loss_file_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training/ResNet18/cifar10/block_1/decoder_4_11/min_loss.txt",
                        help="Save log file")
    ...
    # initialize figure
    f, a = plt.subplots(8, 5, figsize=(24, 12))
                ...
                with open(args.loss_file_path, "w")as f4:
                    for epoch in range(0, args.epochs):
                        ...
                        for batchSize, images_clean, images_adv, labels in load_images(args.input_dir_trainSet,
                            ...
                            # forward + backward
                            outputs_cla, outputs_ae, out_3, out_4 = model(images_clean)
                            # if (epoch // 10) % 2 == 1:
                            if epoch == 555:
                                # Optimization
                        ...
                        if (epoch + 1) % 1 == 0:
                            ...
                            with torch.no_grad():
                                ...
                                for batchSize, images_clean, images_adv, labels in load_images(args.input_dir_testSet,
                                    ...
                                    # 测试clean数据集的测试集
                                    outputs_clean_cla, outputs_clean_ae, out_clean_3, out_clean_4 = model(images_clean)
                                    ...
                                    # 测试adv数据集的测试集
                                    outputs_adv_cla, outputs_adv_ae, out_adv_3, out_adv_4 = model(images_adv)
                                    ...
                                    if batch_id_test == 4:
                                        ...
                                        for i in range(5):
                                            a[1][i].clear()
                                            a[1][i].imshow(outputs_clean_ae.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[1][i].set_xticks(())
                                            a[1][i].set_yticks(())
                                            a[1][i].set_title("Clean Images Decoded" + "[" + str(i + 1) + "]")

                                        for i in range(5):
                                            a[2][i].clear()
                                            a[2][i].imshow(out_clean_3.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[2][i].set_xticks(())
                                            a[2][i].set_yticks(())
                                            a[2][i].set_title("Clean Images H Encoded" + "[" + str(i + 1) + "]")

                                        for i in range(5):
                                            a[3][i].clear()
                                            a[3][i].imshow(out_clean_4.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[3][i].set_xticks(())
                                            a[3][i].set_yticks(())
                                            a[3][i].set_title("Clean Images L Decoded" + "[" + str(i + 1) + "]")

                                        for i in range(5):
                                            a[4][i].clear()
                                            a[4][i].imshow(images_adv.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[4][i].set_xticks(())
                                            a[4][i].set_yticks(())
                                            a[4][i].set_title("Adv Images" + "[" + str(i + 1) + "]")

                                        for i in range(5):
                                            a[5][i].clear()
                                            a[5][i].imshow(out_adv_3.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[5][i].set_xticks(())
                                            a[5][i].set_yticks(())
                                            a[5][i].set_title("Adv Images H Eecoded" + "[" + str(i + 1) + "]")

                                        for i in range(5):
                                            a[6][i].clear()
                                            a[6][i].imshow(out_adv_4.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[6][i].set_xticks(())
                                            a[6][i].set_yticks(())
                                            a[6][i].set_title("Adv Images H Decoded" + "[" + str(i + 1) + "]")

                                        for i in range(5):
                                            a[7][i].clear()
                                            a[7][i].imshow(outputs_adv_ae.cpu().data.numpy()[i, :, :, :].transpose((1, 2, 0)))
                                            a[7][i].set_xticks(())
                                            a[7][i].set_yticks(())
                                            a[7][i].set_title("Adv Images Decoded" + "[" + str(i + 1) + "]")
                                        plt.suptitle("Example" + str(epoch + 1))
                                        plt.draw()
                                        plt.pause(0.05)
                                        if (epoch + 1) % 5 == 0:
                                            img = plt.gcf()
...
if __name__ == "__main__":
    ...
    #                      + str(190) + ".pth"
    joint_train(best_cla_model_path, best_ae_model_path, "cuda:0")
