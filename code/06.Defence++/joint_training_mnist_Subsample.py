...
def load_images(input_dir, batch_shape_clean, batch_shape_adv):
    ...
    for j in range(len(images_clean_list)):
        ...
        gt[:, :, 0], gt[:, :, 1], gt[:, :, 2] = image_clean_j[0, :, :], image_clean_j[0, :, :], image_clean_j[0, :, :]
        gt_4_j = imresize(gt, (14, 14))
        # cv2.imshow("gray", gt_4_j)
        # cv2.waitKey(0)
        gt_3_j = imresize(gt_4_j, (7, 7))
        gt_2_j = imresize(gt_3_j, (4, 4))
        gt_1_j = imresize(gt_2_j, (2, 2))
        gt_4[idx, 0, :, :] = gt_4_j[:, :, 0]
        gt_3[idx, 0, :, :] = gt_3_j[:, :, 0]
        gt_2[idx, 0, :, :] = gt_2_j[:, :, 0]
        gt_1[idx, 0, :, :] = gt_1_j[:, :, 0]
...
def joint_train(best_cla_model_path, block, device_used):
    ...
    parser.add_argument("--input_dir_trainSet", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training_Subsample_0.01(14,7)/ResNet18/mnist/train/train.pkl",
                        help="data set dir path")
    parser.add_argument("--input_dir_testSet", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training_Subsample_0.01(14,7)/ResNet18/mnist/test/test.pkl",
                        help="data set dir path")
    parser.add_argument("--model_path", type=str,
                        default="D:/python_workplace/resnet-AE/checkpoint/Joint_Training_Subsample_0.01(14,7)/ResNet18/mnist/block_"
                                + str(block) + "/model/",
                ...
                with open(args.loss_file_path, "w") as f4:
                    for epoch in range(1, args.epochs + 1):
                        ...
                        for batchSize, gt_1, gt_2, gt_3, gt_4, images_clean, images_adv, labels in load_images(args.input_dir_trainSet,
                            ...
                            # loss = loss_cla + 0.8 * loss_ae + 0.01 * (loss_block_1 + loss_block_2 + loss_block_3 + loss_block_4)
